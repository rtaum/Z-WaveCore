...
{
    public class MultiChannel : CommandBase
    {
        ...
        public MultiChannel(ZWaveNode node)
        ...
        public async Task BinarySwitchSet(byte endPointId, bool value)
        {
            ...
            await Channel.Send(Node,
                new Command(Class, command.Encap, controllerID, endPointId,
                Convert.ToByte(CommandClass.SwitchBinary),
                Convert.ToByte(SwitchBinary.command.Set),
                value ? (byte)0xFF : (byte)0x00), CancellationToken.None);
        }
        ...
        private static ConstructorInfo GetCommandClassConstructor<T>() where T : EndpointSupportedCommandClassBase
        {
            foreach (ConstructorInfo ctr in typeof(T).GetTypeInfo().DeclaredConstructors)
            {
                ...
                if (constructorParameters.Length == 2 && constructorParameters[0].ParameterType == typeof(ZWaveNode) && constructorParameters[1].ParameterType == typeof(byte))
                {
                    return ctr;
                }
            };
