...
namespace ZWaveCore.Core
{
    public class ZWaveNode
    {
        ...
        private List<CommandBase> _commandClasses = new List<CommandBase>();
        ...
        public ZWaveNode(byte nodeID, ZWaveController contoller)
        {
            NodeID = nodeID;
        ...
        {
            lock (typeof(ZWaveNode)) { return functionID = (byte)((functionID % 255) + 1); }
        }
        ...
        public T GetCommandClass<T>() where T : ICommand
        {
        ...
        public Task<ZWaveNode[]> GetNeighbours()
        {
            return GetNeighbours(CancellationToken.None);
        }

        public async Task<ZWaveNode[]> GetNeighbours(CancellationToken cancellationToken)
        {
            var nodes = await Controller.GetNodes();
            var results = new List<ZWaveNode>();
